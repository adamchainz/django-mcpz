...
django-mcpz implements the server side of MCP revision 2026-07-28 over the `Streamable HTTP transport <https://modelcontextprotocol.io/specification/2026-07-28/basic/transports/streamable-http>`__, plus the stateless server mode of the 2025 revisions (see :ref:`protocol-legacy`).
The 2026-07-28 revision made the protocol stateless, with no initialization handshake, no sessions, and no server-sent event streams, which is what lets a plain synchronous view serve it in full, under WSGI, with no need for ASGI.
...
* ``tools/list``, returning all registered tools in registration order (deterministic, per the specification’s caching recommendation).
...
Servers with the default ``minimum_protocol_version`` also serve the 2025 revisions, as covered in :ref:`server-legacy`.
A request without an ``MCP-Protocol-Version`` header, or with one naming a 2025 revision, is handled in those revisions’ stateless server mode:

* ``initialize`` is answered with the requested protocol version (2025-11-25, 2025-06-18, or 2025-03-26), or the latest of those if the request named another, and the same capabilities and ``instructions`` as ``server/discover``.
  No ``Mcp-Session-Id`` header is issued, so clients treat the server as stateless.
* ``notifications/initialized`` is acknowledged with HTTP 202, and ``ping`` answered with an empty result.
* ``tools/list`` and ``tools/call`` work as for 2026-07-28, without the ``_meta`` fields and ``Mcp-*`` header requirements that revision added.
